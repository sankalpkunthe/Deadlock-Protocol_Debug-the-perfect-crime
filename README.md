...
- Add `start`/`dev` scripts to `backend/package.json` to simplify running the server.
- Add sandboxing (Docker / Firecracker / gVisor) before exposing code execution publicly.
- Add CI checks and unit/integration tests for critical endpoints.

## Contributing
- Open an issue or create a PR. Follow existing code style in `frontend/src` and backend patterns in `backend/`.

---
If you want, I can: (1) add `start` and `dev` scripts to the `backend/package.json`, (2) create a short CONTRIBUTING.md, or (3) commit these changes — tell me which. 
